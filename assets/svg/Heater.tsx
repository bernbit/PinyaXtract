...
const HeaterBlade = Animated.createAnimatedComponent(Path);

const Heater = ({ width = 291, height = 215 }: SVGPropsType) => {
  const { heaterState, setHeaterState, toggleHeaterState_SVG, heaterManual } =
    useGlobal();
  const isFocused = useIsFocused();
  const heaterRotation = useSharedValue<number>(0);
